...
<div class="bg-white p-4 md:p-6 rounded-xl shadow">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
        <h1 class="text-xl md:text-2xl font-bold">Kelola Orang Tua</h1>
    </div>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($orangTuas as $ortu)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-2">{{ $loop->iteration + ($orangTuas->currentPage() - 1) * $orangTuas->perPage() }}</td>
        ...
        @forelse($orangTuas as $ortu)
            <div class="border rounded-lg p-4 shadow-sm">
                <div class="flex justify-between items-start">
                    ...
                    <span class="text-xs text-gray-400 ml-2">#{{ $loop->iteration + ($orangTuas->currentPage() - 1) * $orangTuas->perPage() }}</span>
                </div>
                <div class="mt-3">
                    <a href="{{ route('admin.orang_tua.edit', $ortu->id) }}"
