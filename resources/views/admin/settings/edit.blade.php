...
<div class="bg-white p-6 rounded-xl shadow">
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="profil_sekolah" rows="4"
                class="w-full border rounded-lg px-4 py-2">{{ old('profil_sekolah', $settings['profil_sekolah'] ?? '') }}</textarea>
        </div>

        {{-- SAMBUTAN KEPALA SEKOLAH (BARU) --}}
        <div>
            <label class="block font-semibold mb-1">Sambutan Kepala Sekolah</label>
            <textarea name="sambutan_kepsek" rows="4"
                class="w-full border rounded-lg px-4 py-2">{{ old('sambutan_kepsek', $settings['sambutan_kepsek'] ?? '') }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="visi" rows="3"
                class="w-full border rounded-lg px-4 py-2">{{ old('visi', $settings['visi'] ?? '') }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="misi" rows="4"
                class="w-full border rounded-lg px-4 py-2">{{ old('misi', $settings['misi'] ?? '') }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="sejarah" rows="5"
                class="w-full border rounded-lg px-4 py-2">{{ old('sejarah', $settings['sejarah'] ?? '') }}</textarea>
        </div>
        ...
        <div>
            ...
            <input type="text" name="nama_kepsek"
                value="{{ old('nama_kepsek', $settings['nama_kepsek'] ?? '') }}"
                class="w-full border rounded-lg px-4 py-2">
        </div>
