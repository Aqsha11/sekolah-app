...
@php
    ...
    $heroImg = !empty($settings['hero_image']) ? asset('storage/settings/' . $settings['hero_image']) : asset('images/sekolah.jpg');

    $profileTitle = 'Profil Sekolah';
    $profileDesc = $settings['profil_sekolah'] ?? 'SMPN 1 Lambandia merupakan sekolah yang berkomitmen memberikan pendidikan berkualitas serta membentuk karakter siswa yang unggul dan berprestasi.';
    $profileImg = !empty($settings['profil_image']) ? asset('storage/settings/' . $settings['profil_image']) : asset('images/sekolah.jpg');
@endphp
...
<section id="bannerSlider"
    ...
    @forelse($banners as $index => $banner)
        <div class="banner-slide absolute inset-0 {{ $index === 0 ? 'opacity-100' : 'opacity-0' }} transition-opacity duration-1000 ease-in-out">
            ...
            <div class="relative z-10 max-w-5xl mx-auto px-4 text-center">
                <div class="flex flex-col items-center justify-center min-h-[85vh] py-20">
                    <div class="space-y-6">
                        <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-500/20 backdrop-blur-sm text-white text-sm font-semibold border border-white/10">
                            <i class="fa-solid fa-school"></i>
                            {{ $schoolName }}
                        </span>
                        <h1 class="text-4xl sm:text-5xl md:text-7xl text-white font-bold leading-tight tracking-tight drop-shadow-lg">
                            {{ $banner->title }}
                        </h1>
                        @if ($banner->subtitle)
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        ...
        @forelse($statistics ?? [] as $stat)
            ...
            <div data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}"
                ...
                <div class="w-14 h-14 mx-auto rounded-xl {{ $colors['bg'] }} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform text-white text-2xl shadow-lg">
                    <i class="fa-solid {{ $colors['icon'] }}"></i>
                </div>

                <h3 class="text-xs text-slate-500 font-semibold uppercase tracking-wider">{{ $stat->label }}</h3>
                <p class="text-3xl font-extrabold text-slate-900 mt-1 group-hover:text-blue-500 transition-colors">{{ $stat->value }}</p>
...
<section class="max-w-7xl mx-auto px-4 md:px-6 pb-20">
    <div class="grid md:grid-cols-2 gap-12 items-center">
        ...
        <div data-aos="fade-left" class="space-y-6">
            ...
            <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-100 text-blue-600 text-sm font-semibold">
                <i class="fa-solid fa-school"></i>
                Profil Sekolah
            </span>
...
<section class="py-20 bg-white relative overflow-hidden">
    ...
    <div class="absolute bottom-0 right-0 w-72 h-72 bg-purple-500/5 rounded-full blur-[100px] pointer-events-none"></div>
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12" data-aos="fade-up">
            ...
            <div>
                <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-100 text-purple-600 text-sm font-semibold">
                ...
                <p class="text-slate-500 mt-2 max-w-2xl">
                    Berbagai pencapaian dan penghargaan yang berhasil diraih oleh siswa maupun sekolah dalam bidang akademik dan non akademik.
                </p>
            </div>
            ...
            @forelse($recentPrestasi as $index => $prestasi)
                <div data-aos="fade-up" data-aos-delay="{{ ($index % 3) * 100 }}"
                    ...
                    <div class="relative h-48 overflow-hidden bg-slate-100">
                        @if ($prestasi->image)
                            <img src="{{ asset('storage/prestasi/' . $prestasi->image) }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-700">
                        @else
                        ...
                        <div class="absolute top-3 left-3">
                            <span class="px-3 py-1 bg-purple-500 text-white text-xs font-semibold rounded-full shadow">PRESTASI</span>
                        </div>
                    </div>

                    <div class="p-6">
                        <h3 class="text-lg font-bold text-slate-900 mb-2 group-hover:text-purple-600 transition line-clamp-2">
                            {{ $prestasi->title }}
                        </h3>
...
<section class="py-20 bg-slate-50 relative overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12" data-aos="fade-up">
            ...
            <div>
                <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-100 text-amber-600 text-sm font-semibold">
                    <i class="fa-solid fa-newspaper"></i>
                    INFORMASI SEKOLAH
                </span>
            ...
            @forelse($recentNews as $index => $news)
                <article data-aos="fade-up" data-aos-delay="{{ $index * 100 }}"
                    ...
                    <div class="relative h-44 overflow-hidden bg-slate-200">
                        @if ($news->image)
                            <img src="{{ asset('storage/berita/' . $news->image) }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                        @else
            ...
            @foreach ($recentFasilitas as $index => $item)
                <div data-aos="fade-up" data-aos-delay="{{ ($index % 3) * 100 }}"
                    ...
                    <div class="relative h-48 overflow-hidden">
                        @if ($item->image)
                            <img src="{{ asset('storage/' . $item->image) }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-700">
                        @else
                        ...
                        @endif
                        <div class="absolute top-3 left-3">
                            <span class="px-3 py-1 bg-blue-500 text-white text-xs font-semibold rounded-full shadow">FASILITAS</span>
                        </div>
                    </div>

                    <div class="p-6">
                        <h3 class="text-lg font-bold text-slate-900 mb-2 group-hover:text-blue-600 transition">{{ $item->name }}</h3>
                        <p class="text-sm text-slate-500 leading-relaxed line-clamp-3">{{ Str::limit(strip_tags($item->description), 120) }}</p>
                    </div>
...
<section class="py-20 bg-slate-50 relative overflow-hidden">
    ...
    <div class="absolute bottom-0 right-0 w-72 h-72 bg-emerald-500/5 rounded-full blur-[100px] pointer-events-none"></div>
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12" data-aos="fade-up">
            ...
            <div>
                <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-100 text-emerald-600 text-sm font-semibold">
                ...
                <p class="text-slate-500 mt-2 max-w-2xl">
                    Didukung tenaga pendidik berpengalaman dan profesional untuk menciptakan pembelajaran modern berkualitas.
                </p>
            </div>
            ...
            @foreach ($recentGuru as $index => $guru)
                <div data-aos="fade-up" data-aos-delay="{{ ($index % 4) * 100 }}"
                    ...
                    <div class="relative h-64 overflow-hidden bg-slate-100">
                        @if ($guru->photo)
                            <img src="{{ asset('storage/guru/' . $guru->photo) }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-700">
                        @else
                            <div class="w-full h-full flex items-center justify-center">
                    ...
                    <div class="p-5 text-center">
                        <h3 class="text-lg font-bold text-slate-900 group-hover:text-emerald-600 transition">{{ $guru->name }}</h3>
...
<section class="py-20 bg-white relative overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12" data-aos="fade-up">
            ...
            <div>
                <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-rose-100 text-rose-600 text-sm font-semibold">
        ...
        <style>
            ...
            .card-home-galeri:hover .card-icon { scale: 0; }
            ...
            .card-home-galeri:hover .card-image { scale: 0; }
        </style>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            @forelse($recentGaleri as $index => $foto)
                <div data-aos="fade-up" data-aos-delay="{{ $index * 100 }}" class="card-home-galeri">
                    ...
                    @if ($foto->image)
                        <img src="{{ asset('storage/galeri/' . $foto->image) }}" alt="{{ $foto->title }}" class="card-image">
                    @else
